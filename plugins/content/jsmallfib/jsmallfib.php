...
//		if(!stristr(str_replace("/", "\\", $dir), str_replace("/", "\\", $_SERVER["DOCUMENT_ROOT"]))) 
		...
		if ($this->filter_list_allow && (count($dirs) || count($files)))
		{
			...
			$filter_list_tr .= "<tr ".($file_filter_pattern_required ? "class='row highlighted' style='border-style:solid'" : "").">"
				...
				."<td colspan='7'>"
				."<table class='filterTable' style='font-size:12px'>"
				."<tr>"
                ...
                else
		{
                    ...
                    $text .= "<td class='fileName' style='font-size:12px'>"
				.$this->makeArrow((isset($_GET["sort_by"]) ? $_GET["sort_by"] : ""), (isset($_GET["sort_as"]) ? $_GET["sort_as"] : ""), "name", $masked_dir, JText::_('file_name'))
                    ...
                    {
			$text .= "<td class='fileSize' style='font-size:12px'>"
			...
			$text .= "<td class='emptyTd'></td>";
                    ...
                    {
                        $text .= "<td class='fileChanged' style='font-size:12px'>"
			...
			foreach ($files as $a_file)
			{
				...
				else
				{
					...
					$text .= "<tr class='row $row_style' onmouseover='this.className=\"row highlighted\"' onmouseout='this.className=\"row $row_style\"'>"
						...
						."	<td class='fileName' style='font-size:14px'>"
						.$file_link
						."	</td>"
						.($this->display_filesize ? "<td class='fileSize' style='font-size:12px'>".$this->fileSizeF($a_file["size"])."</td>" : "<td class='emptyTd'></td>")
						.($this->display_filedate ? "<td class='fileChanged' style='font-size:12px'>".$this->fileChanged($a_file["changed"])."</td>" : "<td class='emptyTd'></td>");
